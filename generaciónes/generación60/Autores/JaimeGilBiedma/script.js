...
fetch('Textos/biografia2.txt') //CAMBIAR ESTO PARA CONFIGURAR
  ...
  .then(texto => {
    ...
    const frasesEnCursiva = /(Compañeros de viaje|Moralidades|Poemas póstumos|Bajo tolerancia|Final de un adiós)/gi; //CAMBIAR ESTO PARA CONFIGURAR
...
fetch('Textos/postumo.txt') //CAMBIAR ESTO PARA CONFIGURAR
  .then(response => response.text())
  .then(texto => {
    const frasesEnNegrita = /(mujeres de la generación del 27|(las Sinsombrero)|Maruja Mallo|Marga Gil-Roësset|María Zambrano|María Teresa León|Josefina de la Torre|Rosa Chacel|Ernestina de Champourcín o Concha Méndez)/gi; //CAMBIAR ESTO PARA CONFIGURAR
    const frasesEnCursiva = /(Poemas póstumos|Alianza y condena|El vuelo de la celebración|Bajo tolerancia|Final de un adiós)/gi; //CAMBIAR ESTO PARA CONFIGURAR
    const frasesSubrayadas = /(texto a subrayar|otro texto)/gi; //CAMBIAR ESTO PARA CONFIGURAR
    ...
    document.getElementById('postumo').innerHTML = textoFinal; //CAMBIAR ESTO PARA CONFIGURAR
}).catch(error => console.error('Error al cargar el archivo:', error));

fetch('Textos/postumo2.txt') //CAMBIAR ESTO PARA CONFIGURAR
  .then(response => response.text())
  .then(texto => {
    const frasesEnNegrita = /(mujeres de la generación del 27|(las Sinsombrero)|Maruja Mallo|Marga Gil-Roësset|María Zambrano|María Teresa León|Josefina de la Torre|Rosa Chacel|Ernestina de Champourcín o Concha Méndez)/gi; //CAMBIAR ESTO PARA CONFIGURAR
    const frasesEnCursiva = /(Poemas póstumos|Alianza y condena|El vuelo de la celebración|Bajo tolerancia|Final de un adiós)/gi; //CAMBIAR ESTO PARA CONFIGURAR
    ...
    document.getElementById('postumo2').innerHTML = textoFinal; //CAMBIAR ESTO PARA CONFIGURAR
}).catch(error => console.error('Error al cargar el archivo:', error));
